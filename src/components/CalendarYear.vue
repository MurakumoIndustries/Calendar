    ...
    <div class="d-flex flex-column h-100">
        <div class="row">
            <div class="col-auto text-start">
                <a class="btn btn-outline-secondary p-0 m-2" :href="'#!' + prevYear">
            ...
            <div class="col-auto text-end">
                <a class="btn btn-outline-secondary p-0 m-2" :href="'#!' + nextYear">
                    <i style="font-size: 3rem;" class="material-icons">chevron_right</i>
                </a>
            </div>
        </div>
        <div class="flex-fill d-flex flex-column" style="height:calc(100% - 4rem);
    overflow-y: auto;
    overflow-x: hidden;">
            <div class="row flex-fill">
                <div v-for="month in 12" :key="month" class="col-12 col-sm-6 col-md-4 col-lg-3 pt-0 pb-4 px-4">
                    ...
                    <div class="row max-width">
                        <div v-for="labelIndex in 7" :key="labelIndex" class="col text-center date-label-container">
                            <span class="date-label" :class="{
                    ...
                    <div v-for="i in rows(month)" :key="i" class="row max-width">
                        <div v-for="j in 7" :key="j" class="col text-center date-num-container" :class="{
                            'not-in-month': !isDateInMonth(month, i, j),
                            'has-actress': isDateInMonth(month, i, j) && hasActress(month, i, j)
                        }">
                            <div v-if="isDateInMonth(month, i, j) && hasActress(month, i, j)" class="actress-icon">
                                <div v-if="actressList[month][i][j].length == 1"
                            ...
                            <div v-else class="date-num" :class="{
...
export default {
    ...
    computed: {
        ...mapState(useStore, ["year", "firstDayOfWeek", "prevYear", "nextYear"]),
        ...mapWritableState(useStore, ["currentActressId"]),
    },
    methods: {
        rows: function (month) {
            var rows = 0;
        ...
        openDetail: function (month, row, col, index) {
            ...
            this.currentActressId = this.actressList[month][row][col][index].id;
        }
    },
    components: {}
};
